...
do
    ...
    if not Rayfield or type(Rayfield) ~= "table" then warn("[VAPE] Rayfield introuvable.") return end
end
...
local S = {
    ...
    ESP_Box3D=false, ESP_HealthBar=false, ESP_Traceline=false,
...
local function mkLine(color, thick)
    ...
    l.Thickness=thick or 1.5; l.Transparency=1; l.Visible=false; return l
end
...
local function newBox3D(adornee, color)
    local sb=Instance.new("SelectionBox"); sb.Color3=color; sb.LineThickness=0.035
    sb.SurfaceTransparency=1; sb.SurfaceColor3=color; sb.Adornee=adornee
    sb.Visible=false; sb.Parent=workspace; return sb
end
...
local cycloneLines={}
for i=1,CYCLONE_SEGS do local l=mkLine(Color3.fromRGB(180,80,255),1.4); l.Transparency=0.8; cycloneLines[i]=l end
...
local function isSafePosition(pos)
    ...
    local ok,res=pcall(workspace.Raycast, workspace, pos+Vector3.new(0,3,0), Vector3.new(0,-50,0), rp)
...
local function applyEntityESP(model)
    ...
    local lbl=makeLabel(head, color); lbl.Text=plr and plr.Name or model.Name
    local box3d=newBox3D(model, color)
    local hbar=newHBar()
    local tline=mkLine(color, 1.5)
    eESP[model]={hl=hl,label=lbl,box3d=box3d,hbar=hbar,tline=tline,root=root,hum=hum,isPlayer=isPlayer,plr=plr}
    model.AncestryChanged:Connect(function()
        if model:IsDescendantOf(workspace) then return end
        local d=eESP[model]
        if d then
            pcall(function() d.hbar.bg:Remove() end); pcall(function() d.hbar.bar:Remove() end)
            pcall(function() d.tline:Remove() end);   pcall(function() d.box3d:Destroy() end)
...
for _,p in ipairs(Players:GetPlayers()) do
    if p~=LP then p.CharacterAdded:Connect(function(c) task.wait(1); applyEntityESP(c) end) end
end
...
-- ==========================================
local KW_SET={}
for _,k in ipairs({"key","keycard","coin","gold","silver","gem","ruby","emerald","diamond",
...
local EXCL_SET={}
for _,k in ipairs({"wall","floor","ceiling","roof","beam","pillar","terrain","ground",
...
local function applyItemESP(obj)
    ...
    local hl=Instance.new("Highlight"); hl.FillColor=S.C_ITEM
    hl.OutlineColor=Color3.fromRGB(255,255,200); hl.FillTransparency=0.35
...
local function aimAt(targetPos, smooth)
    ...
    if smooth and smooth<1 then Cam.CFrame=Cam.CFrame:Lerp(targetCF,smooth)
...
local function calcBlinkDest(myHRP)
    ...
    local flatLook=Vector3.new(myLook.X,0,myLook.Z)
    local dir=flat.Magnitude>0.1 and flat.Unit or (flatLook.Magnitude>0.01 and -flatLook.Unit or Vector3.new(0,0,-1))
    local perp=Vector3.new(-dir.Z,0,dir.X)
    local dist=S.BlinkDist
    local candidates={
        closest.Position+dir*dist+Vector3.new(0,0.5,0),
        closest.Position+perp*dist+Vector3.new(0,0.5,0),
        closest.Position-perp*dist+Vector3.new(0,0.5,0),
        closest.Position+Vector3.new(0,0.5,0),
    }
    for _,c in ipairs(candidates) do
        local safe,g=isSafePosition(c)
...
local function doBlink()
    ...
    _blinkLast=now
    if not blinkDest then Notify("Blink","Aucun joueur a proximite",2) return end
...
do
    ...
    TESP=mkTab("ESP",4483362458); TAim=mkTab("Aimbot",4483362458)
    TMov=mkTab("Movement",4483362458); TMsc=mkTab("Misc",4483362458)
    if not (TESP and TAim and TMov and TMsc) then warn("[VAPE] Tab creation failed") return end
end
...
TESP:CreateToggle({Name="Box 3D",CurrentValue=false,Callback=function(v)
    S.ESP_Box3D=v
...
TESP:CreateColorPicker({Name="Couleur Players",Color=S.C_PLAYER,Callback=function(v)
    ...
    for _,d in pairs(eESP) do if d.isPlayer then
        d.hl.FillColor=v; d.hl.OutlineColor=v; d.label.TextColor3=v; d.box3d.Color3=v; d.tline.Color=v
    end end
end})
...
TAim:CreateDropdown({Name="Target Part",Options={"Head","HumanoidRootPart","UpperTorso","Torso","LowerTorso"},
CurrentOption={"Head"},Callback=function(opt)
    local v=type(opt)=="table" and opt[1] or tostring(opt)
    if v and v~="" and v~="nil" then S.AimPart=v end
end})
TAim:CreateSlider({Name="Smoothing",Range={1,100},Increment=1,CurrentValue=12,
...
local TPDrop=TMov:CreateDropdown({Name="Choisir un joueur",Options=getPlayerNames(),
CurrentOption={},MultipleOptions=false,Callback=function(opt)
    local v=type(opt)=="table" and opt[1] or tostring(opt)
...
TMsc:CreateToggle({Name="Infinite Stamina",CurrentValue=false,Callback=function(v) _G.Vape_InfStamina=v end})
...
-- ==========================================
if isMobile then
    local sg=Instance.new("ScreenGui"); sg.Name="VapeMobileHUD"; sg.ResetOnSpawn=false; sg.IgnoreGuiInset=true
    safeParent(sg)
    ...
    local bM,fM=mkBtn("VAPE",Color3.fromRGB(0,255,150)); fM.Size=UDim2.new(0,62,0,38); fM.Position=UDim2.new(1,-76,0,56)
...
do
    ...
    local lbl=Instance.new("TextLabel",sg); lbl.Size=UDim2.new(0,220,0,28); lbl.Position=UDim2.new(1,-232,0,10)
    lbl.BackgroundColor3=Color3.fromRGB(10,10,14); lbl.BackgroundTransparency=0.1; lbl.BorderSizePixel=0
    lbl.TextColor3=Color3.fromRGB(255,255,255); lbl.Font=Enum.Font.GothamBold; lbl.TextSize=12
    lbl.TextXAlignment=Enum.TextXAlignment.Center; lbl.Text="VAPE UNIVERSAL | FPS: --"
    Instance.new("UICorner",lbl).CornerRadius=UDim.new(0,6)
    local st=Instance.new("UIStroke",lbl); st.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    ...
    RS.RenderStepped:Connect(function()
        ...
        if now-last>=1 then lbl.Text=string.format("VAPE UNIVERSAL | FPS: %d",fc); fc=0; last=now end
    end)
end
...
RS.Stepped:Connect(function()
    ...
    elseif _noclipWasOn then
        _noclipWasOn=false
        for _,p in ipairs(char:GetDescendants()) do if p:IsA("BasePart") and not p.CanCollide then p.CanCollide=true end end
...
RS.Heartbeat:Connect(function()
    ...
    local hum=char and char:FindFirstChildOfClass("Humanoid"); local myPos=hrp and hrp.Position

    -- Fly
    if S.Fly and Fly.bv and hrp then
        hum.PlatformStand=true; local cam=Cam.CFrame; local mv=Vector3.zero
        ...
        if UIS:IsKeyDown(Enum.KeyCode.LeftControl) or UIS:IsKeyDown(Enum.KeyCode.LeftShift) then mv-=Vector3.new(0,1,0) end
        if isMobile and hum.MoveDirection.Magnitude>0 then
            local md=hum.MoveDirection; mv+=cam.LookVector*(-md.Z)+cam.RightVector*md.X
        end
    ...
    if char and _G.Vape_InfStamina then
        for _,v in ipairs(char:GetDescendants()) do
            if v:IsA("NumberValue") and v.Name:lower():find("stamina") then v.Value=v.MaxValue or 999999 end
        end
    end
    ...
    for model,d in pairs(eESP) do
        ...
        if active and rootPos then
            ...
            d.label.Text=S.ESP_ShowDist and (name.." ["..dist.."m] HP:"..hp) or (name.." HP:"..hp)
            d.label.Visible=true
        else
            d.label.Visible=false
        end

        -- Health bar (couleur rouge->vert, jamais noir)
        if _drawingOk and S.ESP_HealthBar and active and rootPos then
            ...
            if tSP.Z>0 then
                ...
                local bx=mSP.X-bh*0.52*0.5-7; local by=math.min(tSP.Y,bSP.Y)
                local hp=(d.hum and type(d.hum.Health)=="number") and d.hum.Health or 100
                local hmax=(d.hum and type(d.hum.MaxHealth)=="number" and d.hum.MaxHealth>0) and d.hum.MaxHealth or 100
        ...
        if _drawingOk and S.ESP_Traceline and active and rootPos then
            ...
            if sp.Z>0 then d.tline.From=tcOrigin; d.tline.To=Vector2.new(sp.X,sp.Y); d.tline.Visible=true
...
Notify("VAPE UNIVERSAL","Loaded by LV_SDZ/MODZ",4)
